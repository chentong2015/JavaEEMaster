...
public class RedisCacheProblems {
    ...
    //
    //    错误方案：延迟双删(间隔之后删除再次cache缓存)，内存队列(针对不同的商品的操作进行串行化执行)，加分布式锁(将并发改成串行执行，性能不行)
    //    解法方案：
    //    1. 分布式的读写锁：读锁(不互斥) + 写锁(互斥)  ==> 针对"读多写少"的业务场景
    //    2. 直接设置缓存的超时时间 expire key 30s    ==> 针对"读多写多"的业务场景，不强制要求"数据库"和"缓存"中的值一致
    //    3. 不用缓存, 避免大量更新缓存的操作           ==> 针对"读多写多"的业务场景，要求"数据库"和"缓存"严格一致
    //       TODO: 直接操作数据库，DB如何支撑高并发的场景 ==> 使用分布式数据库TiDB, 替代MySQL数据库
